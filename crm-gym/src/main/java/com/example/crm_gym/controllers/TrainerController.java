...
public class TrainerController {
    ...
    @ApiOperation(value = "Get trainer workload", response = Double.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successfully retrieved trainer workload"),
            @ApiResponse(code = 400, message = "Invalid input data"),
            @ApiResponse(code = 404, message = "Trainer not found"),
            @ApiResponse(code = 500, message = "Application failed to process the request")
    })
    public ResponseEntity<Double> getTrainerMonthlyDuration(
        ...
        String transactionId = TransactionLogger.generateTransactionId();
        TransactionLogger.logRequestDetails(transactionId, request.getMethod(), request.getRequestURI(), request.getParameterMap());
        double totalDuration = trainerService.getTrainerMonthlyDuration(username, year, month, transactionId);
